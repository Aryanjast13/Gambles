...
const MinesGame = () => {
    ...
    useEffect(() => {
        const fetchUserBetsByGame = async () => {
            try {
                setLoadingBets(true)
                const response = await api.get(`/api/bet/fetch-user-bet-by-game?gameId=${gameId || "6866a603abe94f3ec5b628b0"}`)
        ...
        const fetchTotalWinAndWinStreakByGame = async () => {
            try {
                ...
                const response = await api.get(
                    `/api/bet/get-user-totalwin-and-winningstreak-by-game?gameId=${gameId || "6866a603abe94f3ec5b628b0"}`
                );
